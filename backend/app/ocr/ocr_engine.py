"""
OCR Engine Implementation for Cheque Processing
================================================

This module provides a clean, testable wrapper around PaddleOCR for
performing optical character recognition on cheque images. It is
designed to be integrated into the `backend/app/ocr` package of the
second project (``ocr2``) and follows the coding standards specified
in the architecture documentation.  The goal is to extract text
elements along with their bounding boxes and confidences in a format
suitable for downstream parsing and template‑based field location.

The implementation below draws heavily on the working OCR logic from
the first attempt (see ``app.py`` in the uploaded files).  In
particular it:

* Instantiates separate PaddleOCR engines for Latin/English and Arabic
  scripts.  This mirrors the first attempt where English and Arabic
  models are cached for reuse.  The selection of which engine to use
  can be made per‑field (e.g. based on the template) or by running
  both and merging the results.
* Normalizes Arabic text (via utilities such as `fix_arabic_text`) and
  digits so that the output is consistent across languages.  You
  should import these helpers from your project’s ``text_utils``.
* Returns a list of dataclass instances (`OCRLine`) with typed
  attributes for text, confidence, bounding box, and center point.
* Provides a simple multi‑crop voting mechanism: given a cropped
  region of interest (ROI) the engine will generate a few padded
  versions of the crop, run OCR on each, and select the highest
  scoring prediction.  This improves robustness to tight or loose
  cropping and is a starting point for the more sophisticated voting
  described in the PRD.

To integrate this into the `ocr2` repository:

1. Place this file in ``backend/app/ocr/ocr_engine.py``.
2. Add ``from .ocr_engine import PaddleOCREngine, MICREngine`` in
   ``backend/app/ocr/__init__.py`` so other modules can import it.
3. Update your service layer (e.g. in ``router.py`` or a Celery
   worker) to call `PaddleOCREngine.ocr_image` after preflight
   processing and before field location.  The returned lines can be
   passed directly to the locator for anchor‑based ROI detection.
4. Write tests under ``backend/tests/ocr/test_ocr_engine.py`` using
   golden images and expected text to validate that the engine meets
   the acceptance criteria defined in ``docs/stories/1.4.story.md``.

Notes
-----

* The MICR engine here is a placeholder.  A real implementation
  should load a model trained on E‑13B or CMC7 fonts and return
  high‑confidence predictions for cheque numbers and bank codes.  It
  should also obey the same API contract as `PaddleOCREngine`.
* This module is deliberately independent of any web or API
  framework.  It accepts NumPy images and returns plain Python data
  structures, making it straightforward to unit test.
"""
